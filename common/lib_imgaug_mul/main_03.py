...
def main(args):
    ...
    if not os.path.exists(os.path.join('bin', 'exp', NOW_TIME)):
        os.mkdir(os.path.join(os.path.join('bin', 'exp', NOW_TIME)))
    
    # call meta paths
    meta_data_path_lst = glob.glob(os.path.join('bin', 'data', 'label_meta', 'meta', TARGET, '*.json'))
    ...
    for q in range(len(meta_data_path_lst)):
        ...
        label = json_load(os.path.join('bin', 'data', 'label_meta', LABEL_PATH[0]))
        img_path_lst = glob.glob(os.path.join('bin', 'data', 'face_img', AGE, DATA_KEY, '*.jpg'))\
        ...
        try:
            ...
            for i in range(len(img_path_lst)):
                ...
                for num in range(NUM):
                    ...
                    if not os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'img')):
                        os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'img'))
                        for age_c in age_category_lst:
                            os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'img', age_c))
                        
                    if not os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, DATA_KEY + f"_{FORMAT}")):
                        os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, DATA_KEY + f"_{FORMAT}"))
                        
                    cv2.imwrite(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, DATA_KEY + f"_{FORMAT}", DATA_KEY + f"_{FORMAT}_{NUM_FORMAT}.jpg"), img_output)
                    ...
                    if not os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'label_meta')):
                        os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'label_meta'))
                        
                    if not os.path.exists(os.path.join('exp', NOW_TIME, 'label_meta', 'meta')):
                        os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'meta'))
                        for age_c in age_category_lst:
                            os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'meta', age_c))
                    
                    with open(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'meta', AGE, EXP_META_FNAME), 'w') as outfile:
                    ...
                    if not os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'labels')):
                        os.mkdir(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'labels'))
                    
                    with open(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'labels', EXP_LABEL_FNAME), 'w', encoding='utf-8') as outfile:
            ...
            '''
            data_key_path = os.path.join('bin', 'data', 'label_meta', 'meta', TARGET, DATA_KEY + '.json')
            new_data_key_path = os.path.join('bin', 'data', 'label_meta', 'meta', 'Fail_lst', f'{DATA_KEY}.json')
            
            shutil.copy(data_key_path, new_data_key_path)
            
            created_meta_lst = glob.glob(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'meta', AGE, f'{DATA_KEY}?*'))
            ...
            for meta_path in created_meta_lst:
                ...
                # remove created img
                if os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, C_DATA_KEY)):
                    if os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, C_DATA_KEY, C_FRAMES)):
                        os.remove(os.path.join('bin', 'exp', NOW_TIME, 'img', AGE, C_DATA_KEY, C_FRAMES))
                # remove created labels
                if os.path.exists(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'labels', C_LABEL_PATH)):
                    os.remove(os.path.join('bin', 'exp', NOW_TIME, 'label_meta', 'labels', C_LABEL_PATH))
                # remove created meta
                os.remove(meta_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t", '--t', type=str, required=True)
