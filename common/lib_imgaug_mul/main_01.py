...
def main(args):
    ...
    for q in range(len(meta_data_path_lst)):
        ...
        try:
            ...
            new_data_key_path = os.path.join('data', 'label_meta', 'meta', 'Fail_lst', f'{DATA_KEY}.json')
            
            shutil.copy(data_key_path, new_data_key_path)
            
            created_meta_lst = glob.glob(os.path.join('exp', NOW_TIME, 'label_meta', 'meta', AGE, f'{DATA_KEY}?*'))
