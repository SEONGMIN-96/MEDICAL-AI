...
class Video_Play(object):
    def __init__(self, config: dict) -> None:
        ...
        self.homo_filter = Preprocess(gamma1=0.3, gamma2=1.5)
        ...
        while cap.isOpened():
            ...
            if not ret:
                ...
                if count % self.cut_frame == 0:
                    ...
                    resized_in = cv2.resize(src=count_frame, dsize=(224,224), interpolation=cv2.INTER_AREA)
                    
                    # homomorphic filter 적용
                    filtered_frame0 = self.homo_filter.homomorphic(frame=resized_in)
                    
                    resized_out = cv2.resize(src=filtered_frame0, dsize=(count_frame.shape[0],count_frame.shape[1]), interpolation=cv2.INTER_AREA)
                    
                    if count == 0:
                        self.frame_lst[1] = resized_out
                    else:
