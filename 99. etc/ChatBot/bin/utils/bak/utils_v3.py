...
def dialogflow_intents(intents_S, intents_path, text, entity_path):
    ...
    for word in words:
        if exist == False:
            if word == '종료' or word == '그만':
                res = 'ans-exit'
            else:
                f = open(os.path.join(intents_path, f'I_{intents_S}.txt'), 'r', encoding='utf-8')
                I_res = f.readline()
                entity = dialogflow_entities(intents_S, entity_path)
                if word in entity:
                    res = I_res
                    exist = True
                elif word not in entity:
                    res = 'ans999'
        elif exist == True:
            pass
...
def clova_voice(text, dir_path):
    ...
    client_id = "7z5rs4345v"
    client_secret = "6BmZuzSrUhJ16m1HJGAYjNcOVNDABV4aaFi476RV"
...
def ans_choice(I_res, output_path, input_path, txt_path):
    ...
    if I_res == "ans-exit":
        response = "감사합니다."
        clova_voice(response, output_path)
