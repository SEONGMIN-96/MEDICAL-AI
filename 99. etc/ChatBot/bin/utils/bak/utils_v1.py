...
def synthesize_text(text, dir_path):
    ...
    with open(dir_path + "response.mp3", "wb") as out:
...
def cov_play(dir_path):
    ...
    audSeg = AudioSegment.from_mp3(dir_path + src)
    audSeg.export(dir_path + dst, format="wav")

    import winsound

    winsound.PlaySound(dir_path + dst , winsound.SND_FILENAME)
...
def categorizing_C_keyword(info_path, f_path):
    ...
    txt_path = info_path + f_path
    cancer_path = info_path + "01_cancer_list.txt"
    ko_list = info_path + "02_ko_list.txt"
    sub_detail_path = info_path + "04_sub_detail.txt"
...
def ans_choice(ans, output_path, input_path, txt_path):
    if ans == "ans-exit":
        response = "감사합니다."
        print(response)
