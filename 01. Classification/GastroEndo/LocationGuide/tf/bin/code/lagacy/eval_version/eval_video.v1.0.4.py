...
class GastricPositionClassificationMain(DataIOStream, VideoProcessing, CreateGastricModel, AnalyzeROC):
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            if not ret:
                ...
                # 추론된 결과값이 'NO'가 될 경우, 앞단에서 먼저 인식된 위치를 사용
                # 만약 맨처음 'NO'가 추론되었다면 그대로 'NO'사용
                ...
                class_NUM = {'ES': 0, 'GE': 1, 'CR': 2, 'BODY': 3, 'AT': 4, 'AG': 5, 'BB': 6, 'SD': 7 , 'NO': 8}
                ...
                new_y_true = tf.keras.utils.to_categorical(np.array(y_true))
                new_y_pred = tf.keras.utils.to_categorical(np.array(y_pred))
                
                # roc curve 생성
                self.ROC_multi_video(n_classes=len(class_NUM), y_true=new_y_true, y_pred=new_y_pred, now_time=self.now_time)
