...
class GastricPositionClassificationMain(DataIOStream, 
    ...
    def run(self):
        ...
        # 적용될 위내시경 로고 이미지 로드
        logo_paths = glob.glob(os.path.join('bin', 'data', 'logo', '*'))
        ...
        while cap.isOpened():
            ...
            if not ret:
                ...
                # class_NUM = {'ES': 0, 'CR': 1, 'BODY': 2, 'AT': 3, 'AG': 4, 'DU': 5}
            ...
            # 후처리를 적용하지 않은 추론 결과값 저장
            none_post_process.append(main_deq[-1])
            ...
            #     logo = logo_lst[main_class_dict[MAX_CLS]][-1]
            #     board = self.bit_operation_s0(board=board, logo=logo)
