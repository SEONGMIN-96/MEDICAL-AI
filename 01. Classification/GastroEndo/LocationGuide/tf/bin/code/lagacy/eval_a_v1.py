...
class GastricPositionClassificationMain(DataIOStream, CreateGastricModel, PerformanceMeasurement, Postprocessing, AnalyzeROC):
    ...
    def run(self):
        ...
        # roc curve
        auc_score = self.ROC_multi_all(n_classes=n_classes, y_true=P_ALL, y_pred=L_ALL, exp_path=self.exp_path, name_lst=list(all_c_dict.keys()))
        ...
        report = classification_report(y_true=label_all, y_pred=pred_all, digits=n_classes, output_dict=True)
        ...
        with open(os.path.join('bin', 'exp', 'sample', 'eval.txt'), 'w') as f:
