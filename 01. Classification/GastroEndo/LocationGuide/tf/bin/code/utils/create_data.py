...
class DataCreateStream():
    ...
    def data_split(self, data_classes: list):
        '''load img path & split train test val 
        '''
        # img path 불러옴
        class_path = os.path.join('bin', 'data', 'src', 'class_v2')
        ...
        for class_object in data_classes:
        ...
        # tqdm
        pbar = tqdm(total=len(data_classes))
        ...
        # 
        all_train_label_lst, train_input_id = self.text_label_categorical(input_label=all_train_label_lst, data_classes=data_classes)
        all_test_label_lst, test_input_id = self.text_label_categorical(input_label=all_test_label_lst, data_classes=data_classes)
        all_val_label_lst, val_input_id = self.text_label_categorical(input_label=all_val_label_lst, data_classes=data_classes)
            
        train_dict = {'input_path': all_train_path_lst, 
                    'input_label': all_train_label_lst, 
                    'input_id': train_input_id, 
                    'input_class' : data_classes
                    }
