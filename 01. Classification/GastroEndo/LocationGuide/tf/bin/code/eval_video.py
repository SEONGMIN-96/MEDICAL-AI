    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            if not ret:
                ...
                # 정량적 평가 진행합니다.
                y_true, y_pred, = [], []
                for i, (yt_box, pred) in enumerate(zip(gt, voting_box)):
                    if pred not in yt_box:
                        y_true.append(L_ALL_CLS[yt_box[-1]])
                    elif pred in yt_box:
                        y_true.append(L_ALL_CLS[pred])
                    y_pred.append(L_ALL_CLS[pred])
                del_idx = []
                # NO는 평가에서 제외하기위해 뺍니다.
                ...
                self.plot_confusion_matrix_video(y_true=y_true, 
                                                 y_pred=y_pred, 
                ...
                report = pd.DataFrame(classification_report(y_true=y_true, 
                                                            y_pred=y_pred, 
                                                            digits=L_ALL_CLS_without_NO.values(), 
                                                            output_dict=True
                ))
                report.to_csv(os.path.join('bin', 'exp_v', self.now_time, 'report.csv'))
                # sensitivity & specificity 생성합니다.
                sen_spe = {}
                for i in range(len(L_ALL_CLS_without_NO)):
                    sen, spe = self.sensitivity_specificity_per_class(y_true=y_true, 
                                                                      y_pred=y_pred, 
                                                                      y_class=i
                    )
                    sen_spe[list(L_ALL_CLS_without_NO.keys())[i]] = {'sensitivity': sen, 'specificity': spe}
                pd.DataFrame(sen_spe).to_csv(os.path.join('bin', 'exp_v', self.now_time, 'sen_spe.csv'))
                break
            else:
...
def main():
    ...
    video_path = "bin/data/video/1.400.20.81.610.108966936682144.9534.1.28.dcm"
    ...
    now_time = f"{d.year}-{d.month}-{d.day}-{d.hour}-{d.minute}-{d.second}-{gpu_num}"
